...
describe("validateLabs", () => {
  it("accepts sbp only", () => {
    const result = validateLabs({ sbp: "120", ldl: "", hdl: "" });
  ...
  it("accepts all three fields", () => {
    const result = validateLabs({ sbp: "130", ldl: "110", hdl: "55" });
    expect(result.success).toBe(true);
    if (result.success) {
  ...
  it("rejects when no fields provided", () => {
    const result = validateLabs({ sbp: "", ldl: "", hdl: "", glucose: "", triglycerides: "" });
  ...
  it("rejects sbp out of range (too low)", () => {
    const result = validateLabs({ sbp: "30", ldl: "", hdl: "" });
    expect(result.success).toBe(false);
  });

  it("rejects sbp out of range (too high)", () => {
    const result = validateLabs({ sbp: "300", ldl: "", hdl: "" });
    expect(result.success).toBe(false);
  });

  it("rejects ldl out of range", () => {
    const result = validateLabs({ sbp: "", ldl: "500", hdl: "" });
    expect(result.success).toBe(false);
  });

  it("rejects hdl out of range", () => {
    const result = validateLabs({ sbp: "", ldl: "", hdl: "5" });
    expect(result.success).toBe(false);
  });

  it("accepts minimum valid values", () => {
    const result = validateLabs({ sbp: "60", ldl: "20", hdl: "10" });
    expect(result.success).toBe(true);
    if (result.success) {
  ...
  it("accepts maximum valid values", () => {
    const result = validateLabs({ sbp: "250", ldl: "400", hdl: "150", glucose: "", triglycerides: "" });
    expect(result.success).toBe(true);
  });

  it("accepts glucose only", () => {
    const result = validateLabs({ sbp: "", ldl: "", hdl: "", glucose: "95", triglycerides: "" });
    expect(result.success).toBe(true);
    if (result.success) {
  ...
  it("accepts all five fields", () => {
    const result = validateLabs({ sbp: "130", ldl: "110", hdl: "55", glucose: "100", triglycerides: "150" });
    expect(result.success).toBe(true);
    if (result.success) {
      expect(result.data.sbp).toBe(130);
      expect(result.data.ldl).toBe(110);
  ...
  it("rejects glucose out of range", () => {
    const result = validateLabs({ sbp: "", ldl: "", hdl: "", glucose: "10", triglycerides: "" });
    expect(result.success).toBe(false);
  });

  it("rejects triglycerides out of range", () => {
    const result = validateLabs({ sbp: "", ldl: "", hdl: "", glucose: "", triglycerides: "5000" });
    expect(result.success).toBe(false);
  });
});
